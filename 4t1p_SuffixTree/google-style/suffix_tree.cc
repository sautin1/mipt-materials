...
SuffixTree::Link::Link(int _target_node_index, int _sample_start_index, int _sample_end_index)
  : target_node_index(_target_node_index), sample_start_index(_sample_start_index),
    sample_end_index(_sample_end_index) {}

bool SuffixTree::Link::Equals(const Link& link) const {
  bool is_equal = (target_node_index == link.target_node_index) &&
                  (sample_start_index == link.sample_start_index);
...
SuffixTree::NodeReference::NodeReference(int _closest_ancestor_node, int _sample_start_index,
...
bool SuffixTree::NodeReference::Equals(const NodeReference& node_reference) const {
  bool is_equal = (closest_ancestor == node_reference.closest_ancestor) &&
                  (sample_start_index == node_reference.sample_start_index);
...
bool SuffixTree::TestAndSplitResult::Equals(const TestAndSplitResult& test_split_result) const {
  return (node_index == test_split_result.node_index) &&
         (reached_endpoint == test_split_result.reached_endpoint);
}

SuffixTree::DepthFirstSearchStackItem::DepthFirstSearchStackItem(
    const Link& _enter_link,
...
void SuffixTree::LinkNodes(int source_node_index, int target_node_index,
                           int sample_start_index, int sample_end_index) {
  char letter = sample_[sample_start_index];
  nodes_[source_node_index].links[letter] = Link(target_node_index, sample_start_index,
                                                 sample_end_index);
}
...
void SuffixTree::CanonicalizeNodeReference(NodeReference* node_reference) const {
  if (node_reference->sample_end_index == node_reference->sample_start_index) {
    ...
    // implicit node
    Link link_from_ancestor = GetLink(node_reference->closest_ancestor,
                                      sample_[node_reference->sample_start_index]);
    while (node_reference->sample_end_index - node_reference->sample_start_index >=
      ...
      node_reference->sample_start_index += link_from_ancestor.sample_end_index;
      node_reference->sample_start_index -= link_from_ancestor.sample_start_index;
      if (node_reference->sample_start_index < node_reference->sample_end_index) {
        link_from_ancestor = GetLink(node_reference->closest_ancestor,
                                     sample_[node_reference->sample_start_index]);
      }
    }
  }
...
SuffixTree::TestAndSplitResult SuffixTree::TestAndSplit(const NodeReference& node_reference) {
  if (node_reference.sample_end_index == node_reference.sample_start_index) {
    // explicit node
    bool has_link = HasLink(node_reference.closest_ancestor,
                            sample_[node_reference.sample_end_index]);
    return TestAndSplitResult(has_link, node_reference.closest_ancestor);
  } else {
    // implicit node
    Link link_from_node = GetLink(node_reference.closest_ancestor,
                                  sample_[node_reference.sample_start_index]);
    int link_letter_index = link_from_node.sample_start_index + node_reference.sample_end_index;
    link_letter_index -= node_reference.sample_start_index;
    ...
    LinkNodes(node_reference.closest_ancestor, split_node,
              link_from_node.sample_start_index, link_letter_index);
    LinkNodes(split_node, link_from_node.target_node_index,
              link_letter_index, link_from_node.sample_end_index);
...
SuffixTree::NodeReference SuffixTree::AddNextLetter(NodeReference active_point) {
  ...
  while (!test_and_split_result.reached_endpoint) {
    ...
    LinkNodes(test_and_split_result.node_index, CreateNode(),
              active_point.sample_end_index, kInfinity);
