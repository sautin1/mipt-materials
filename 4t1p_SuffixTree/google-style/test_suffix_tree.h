...
protected:
	...
	void CheckCorrectDummy() {
		...
		ASSERT_EQ(suffix_tree_.dummy_, 0);
		for (size_t letter_index = 0; letter_index < suffix_tree_.sample_.size(); ++letter_index) {
			ASSERT_NO_THROW(suffix_tree_.nodes_[suffix_tree_.dummy_].links.at(suffix_tree_.sample_[letter_index]));
		}
		ASSERT_EQ(suffix_tree_.nodes_[0].suffix_link_node_index, -1);
	}
	...
	void CheckCorrectRoot() {
		...
		ASSERT_EQ(suffix_tree_.root_, 1);
		for (size_t letter_index = 0; letter_index < suffix_tree_.sample_.size(); ++letter_index) {
			ASSERT_NO_THROW(suffix_tree_.nodes_[suffix_tree_.root_].links.at(suffix_tree_.sample_[letter_index]));
		}
		ASSERT_EQ(suffix_tree_.nodes_[1].suffix_link_node_index, 0);
	}
...
TEST_F(SuffixTreeTest, InitDummyTest) {
	...
	size_t old_link_quantity = suffix_tree_.nodes_[suffix_tree_.dummy_].links.size();
	suffix_tree_.sample_ += suffix_tree_.sample_;
	for (size_t sample_start_index = old_link_quantity; sample_start_index < suffix_tree_.sample_.size(); ++sample_start_index) {
		SuffixTree::Link old_link = suffix_tree_.nodes_[suffix_tree_.dummy_].links[suffix_tree_.sample_[sample_start_index]];
		suffix_tree_.InitDummy(sample_start_index, sample_start_index + 1);
		SuffixTree::Link new_link;
		ASSERT_NO_THROW(new_link = suffix_tree_.nodes_[suffix_tree_.dummy_].links.at(suffix_tree_.sample_[sample_start_index]));
		ASSERT_TRUE(old_link.Equals(new_link));
		ASSERT_EQ(suffix_tree_.nodes_[suffix_tree_.dummy_].links.size(), old_link_quantity);
	}
}
...
TEST_F(SuffixTreeTest, InitTreeTest) {
	...
	CheckCorrectDummy();
	CheckCorrectRoot();
	ASSERT_TRUE(suffix_tree_.active_point_.Equals(SuffixTree::NodeReference(suffix_tree_.root_, 0, 0)));
}
...
TEST_F(SuffixTreeTest, GetLinkTest) {
	suffix_tree_.nodes_[suffix_tree_.root_].links.clear();
	suffix_tree_.nodes_[suffix_tree_.dummy_].links.clear();
	ASSERT_THROW(suffix_tree_.GetLink(suffix_tree_.root_, suffix_tree_.sample_[0]), std::out_of_range);
	suffix_tree_.LinkNodes(suffix_tree_.root_, suffix_tree_.dummy_, 0, 1);
	SuffixTree::Link link;
	ASSERT_NO_THROW(link = suffix_tree_.GetLink(suffix_tree_.root_, suffix_tree_.sample_[0]));
	ASSERT_TRUE(link.Equals(SuffixTree::Link(suffix_tree_.dummy_, 0, 1)));
	ASSERT_THROW(suffix_tree_.GetLink(suffix_tree_.dummy_, suffix_tree_.sample_[0]), std::out_of_range);
	suffix_tree_.nodes_[suffix_tree_.root_].links.erase(suffix_tree_.sample_[0]);
	ASSERT_THROW(suffix_tree_.GetLink(suffix_tree_.root_, suffix_tree_.sample_[0]), std::out_of_range);
}
...
TEST_F(SuffixTreeTest, CanonicalizeNodeReferenceNormalTest) {
	...
	for (size_t string_end_index = 0; string_end_index <= suffix_tree_.sample_.size(); ++string_end_index) {
...
TEST_F(SuffixTreeTest, TestAndSplitTest) {
	...
	ASSERT_EQ(suffix_tree_.nodes_.size() - old_nodes_size, 1); // one node was created because of split
	SuffixTree::Link first_link_part, second_link_part;
	ASSERT_NO_THROW(first_link_part = suffix_tree_.nodes_[0].links.at(suffix_tree_.sample_[first_sample_part.size()]));
	...
	ASSERT_NO_THROW(second_link_part = suffix_tree_.nodes_.back().links.at(suffix_tree_.sample_[sample_split_index]));
...
TEST_F(SuffixTreeTest, GetLinkIteratorTest) {
	...
	SuffixTree::LinkMapConstIterator link_map_it = suffix_tree_.GetLinkIterator(suffix_tree_.dummy_, 'a');
...
public:
	...
	LexicalDFSTraversalVisitor(const SuffixTree& suffix_tree_)
		: suffix_tree_(suffix_tree_), traversal_() {}
