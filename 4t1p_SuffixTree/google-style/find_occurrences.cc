...
FindOccurrencesTraversalVisitor::FindOccurrencesTraversalVisitor(const SuffixTree& suffix_tree, const std::string& pattern)
...
LinkMapConstIterator FindOccurrencesTraversalVisitor::ChooseNextNeighbour(int active_node, const LinkMapConstIterator& link_map_begin_it,
	...
	if (pattern_end_node_ == -1) {
		...
		LinkMapConstIterator link_map_letter_it = suffix_tree_.GetLinkIterator(active_node, pattern_[pattern_index_++]);
