...
private:
    ...
    std::vector<Vector> filter_by_belief(const Mat& image_current, const std::vector<Vector>& motion_vectors) const;
    double calc_belief_function(const Vector& vector, const Mat& block) const;
