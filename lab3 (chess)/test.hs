...
main = do
	...
	putStrLn "\nBest white opening:"
	let ww = Trie.reverseResult $ Trie.traverse (Trie.winOpenVisitWithLast Trie.whiteTurn Trie.whiteProb) Trie.goDeeper ([], 0.0) ([], 0) trie
	print $ ww
	print $ Trie.lookup (fst ww) trie

	putStrLn "\nBest black opening:"
	let ww1 = Trie.reverseResult $ Trie.traverse (Trie.winOpenVisitWithLast Trie.blackTurn Trie.blackProb) Trie.goDeeper ([], 0.0) ([], 0) trie
	print $ ww1
	print $ Trie.lookup (fst ww1) trie

	putStrLn "\nBest opening for white:"
	let ww2 = Trie.reverseResult $ Trie.traverse (Trie.winOpenVisit Trie.whiteProb) Trie.goDeeper ([], 0.0) ([], 0) trie
	print $ ww2
	print $ Trie.lookup (fst ww2) trie

	putStrLn "\nBest opening for black:"
	let ww3 = Trie.reverseResult $ Trie.traverse (Trie.winOpenVisit Trie.blackProb) Trie.goDeeper ([], 0.0) ([], 0) trie
	print $ ww3
