...
public:
	...
	friend std::string longestCommonSubstring(const std::string& firstString, const std::string& secondString);
};

std::string longestCommonSubstring(const std::string& firstString, const std::string& secondString);
...
std::string longestCommonSubstring(const std::string& firstString, const std::string& secondString) {
	SuffixArray suffixArrayObject(firstString + "$" + secondString, SuffixArray::SUFFIX_ARRAY);
	size_t firstStringEndIndex = firstString.size() - 1;
	size_t secondStringBeginIndex = firstString.size() + 1;
	size_t lcsSize = 0;
	size_t lcsBeginIndex = 0;
	for (size_t suffixPairIndex = 0; suffixPairIndex < suffixArrayObject.sourceString.size() - 1; ++suffixPairIndex) {
		size_t leftSuffixIndex  = std::min(suffixArrayObject.suffixArray[suffixPairIndex], suffixArrayObject.suffixArray[suffixPairIndex + 1]);
		size_t rightSuffixIndex = std::max(suffixArrayObject.suffixArray[suffixPairIndex], suffixArrayObject.suffixArray[suffixPairIndex + 1]);
		if (leftSuffixIndex <= firstStringEndIndex && rightSuffixIndex >= secondStringBeginIndex &&
				suffixArrayObject.lcp[suffixPairIndex] > (long)lcsSize) {
			lcsSize = suffixArrayObject.lcp[suffixPairIndex];
			lcsBeginIndex = leftSuffixIndex;
		}
	}
	std::string lcs = firstString.substr(lcsBeginIndex, lcsSize);
	return lcs;
}
...
int main()
{
	size_t speechSize;
	std::cin >> speechSize;
	std::string firstSpeech;
	std::cin >> firstSpeech;
	std::string secondSpeech;
	std::cin >> secondSpeech;
	std::string lcs = longestCommonSubstring(firstSpeech, secondSpeech);
	std::cout << lcs << "\n";
	return 0;
}
