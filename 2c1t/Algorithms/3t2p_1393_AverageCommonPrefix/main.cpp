...
public:
	...
	unsigned long long LCPSum() const;
};
...
unsigned long long SuffixArray::LCPSum() const {
	unsigned long long result = 0;
	for (size_t suffixPairIndex = 0; suffixPairIndex < lcp.size()-1; ++suffixPairIndex) {
		result += lcp[suffixPairIndex];
	}
	return result;
}
...
int main()
{
	...
	std::string string;
	std::cin >> string;
	SuffixArray suffixArray(string, SuffixArray::SHIFT_ARRAY);
	unsigned long long lcpSum = suffixArray.LCPSum();
	double result = (double) lcpSum / (string.size() - 1);
